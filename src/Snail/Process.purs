...
-- | the hood.
exec :: NonEmpty Array String -> Snail String
exec (cmd :| as) = do
  res <- execFileImp cmd as
  liftEffect $ toString UTF8 res.stdout

rawImpl :: String -> Snail CP.ExecResult
...
raw :: String -> Snail String
raw cmd = do
  res <- rawImpl cmd
  liftEffect $ toString UTF8 res.stdout

foreign import unref :: CP.ChildProcess -> Script Unit

-- | Send a process into the background.
fork :: NonEmpty Array String -> Snail Unit
fork (cmd :| as)
...
run :: NonEmpty Array String -> Snail Unit
run (cmd :| as) = do
